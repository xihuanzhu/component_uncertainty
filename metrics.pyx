...
def segment_search( int i, int j, unsigned char[:,:] seg, short int seg_ind, np.ndarray marked_array, np.ndarray flag_array,
  ...
  if seg[i,j] < 255:
    ...
    if n_bd > 0:
      ...
      metrics["S_rel"   ][-1] = float( n_in + n_bd ) / (float(n_bd) + 511 * 4)
      metrics["S_rel_in"][-1] = float( n_in ) / (float(n_bd) + 511 * 4)
...
def test_metrics():
  ...
  # probs = np.random.random((64, 64 ,2))
  # gt = np.random.random((64, 64))
  # img = np.random.random((64, 64))
  # ii = 22
  # gt = np.load(f"/raid/zhuxihuan/data/body/expe6/train_data/GT/{ii}.npy")
  # img = np.load(f"/raid/zhuxihuan/data/body/expe6/train_data/IMG/{ii}.npy")
  # probs = np.load(f"/raid/zhuxihuan/data/body/expe6/train_data/probs/{ii}.npy")
  # gt = np.load(f"/raid/zhuxihuan/data/body/expe5/train_data/GT/{ii}.npy")
  # img = np.load(f"/raid/zhuxihuan/data/body/expe5/train_data/IMG/{ii}.npy")
  # probs = np.load(f"/raid/zhuxihuan/data/body/expe5/train_data/probs/{ii}.npy")
