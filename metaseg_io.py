...
def probs_gt_load( i ):
  ...
  if os.path.exists(get_save_path_probs_i(i)):
    probs = np.load(get_save_path_probs_i(i))
  if os.path.exists(get_save_path_GT_i(i)):
    gt = np.load(get_save_path_GT_i(i))
  file_name = str(i) + ".npy"
  return probs, gt, file_name
...
def pixel_metrics_dump( metrics, i ):

  dump_path = get_save_path_pixel_metrics_i( i )
  dump_dir  = os.path.dirname( dump_path )
...
def metrics_load( i , train=True):
  read_path = get_save_path_metrics_i( i , train)
...
def pixel_components_dump( components, i ):

  dump_path = get_save_path_pixel_components_i( i )
  dump_dir  = os.path.dirname( dump_path )
...
def components_load( i , train=True):
  
  read_path = get_save_path_components_i( i , train)
  components = pickle.load( open( read_path, "rb" ) )
  
  return components

def pixel_components_load( i , train=True):
  
  read_path = get_save_path_pixel_components_i( i , train)
  components = pickle.load( open( read_path, "rb" ) )
  
  return components

def get_save_path_pixel_components_i( i , train=True):
  if train:
    return metaseg.get("PIXEL_COMPONENTS_DIR") + "components" + str(i) +".p"
  else:
    return metaseg.get("VAL_PIXEL_COMPONENTS_DIR") + "components" + str(i) +".p"

def get_save_path_metrics_i( i , train=True):
  if train:
    return metaseg.get("METRICS_DIR") + "metrics" + str(i) +".p"
  else:
    return metaseg.get("VAL_METRICS_DIR") + "metrics" + str(i) +".p"

def get_save_path_pixel_metrics_i( i , train=True):
  if train:
    return metaseg.get("PIXEL_METRICS_DIR") + "metrics" + str(i) +".p"
  else:
    return metaseg.get("VAL_PIXEL_METRICS_DIR") + "metrics" + str(i) +".p"


def get_save_path_components_i( i , train=True):
  if train:
    return metaseg.get("COMPONENTS_DIR") + "components" + str(i) +".p"
  else:
    return metaseg.get("VAL_COMPONENTS_DIR") + "components" + str(i) +".p"



def get_save_path_probs_i( i ):
  
  return metaseg.get("PROBS_DIR") + str(i) +".npy"

def get_save_path_GT_i(i):
  return metaseg.get("GT_DIR") + str(i) + ".npy"

def get_save_path_IMG_i(i):
  return metaseg.get("IMG_DIR") + str(i) + ".npy"
